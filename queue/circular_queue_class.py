...
class CircularQueueOneSlotEmpty:
    ...
    def is_full(self):
        return (self.rear + 1) % self.N == self.front # front가 rear 다음에 있으면 포호상태


    def enqueue(self, item):
        if not self.is_full():
            self.rear = (self.rear + 1) % self.N # rear 를 먼저 하나 증가 (시계방향)
    ...
    def dequeue(self):
        if not self.is_empty():
            self.front = (self.front + 1) % self.N # front 를 먼저 하나 증가 (시게방향)
